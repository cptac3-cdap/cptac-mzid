    ...
    def from_rows(self,rows):
        ...
        while True:
            try:
                ...
                break
            i += 1
            self.therows.append(map(r.get,self.headers_))
            if i >= self.maxrows:
                tfd,tfn = tempfile.mkstemp()
                os.close(tfd)
                # print >>sys.stderr, "Writing out file:",tfn,"...",
                wh = open(tfn,'wb')
                map(lambda r: dump(r,wh,HIGHEST_PROTOCOL),self.therows)
                wh.close()
...
class OutOfCoreSortedTable:
    ...
    def rows(self):
		...
		if not(self.nodupkeys and lastk and k == lastk):
		  ...
		  lastk = k
              ...
              for k,r in self.therows:
		if not(self.nodupkeys and lastk and k == lastk):
                  yield dict(zip(self.headers_,k))
		  lastk = k
            return
	...
	thelastrow = None
        while True:
            ...
            k = self.key_(r)
	    if not self.keyisrow:
		therow = (k,map(r.get,self.headers_))
	    else:
		therow = (k,None)
