...
for f in allargs:
  ...
  try:
    ...
    markedbad = True
    if verbose:
        print >>sys.stderr, "Checking", f, "BAD (can't find sourceFile block.)"
    else:
	print f
    raise
  if markedbad:
  ...
  if f.endswith('.gz'):
    try:
      ...
      markedbad = True
      if verbose:
        print >>sys.stderr, "Checking",f, "BAD"
      else:
        print f
      raise
  else:
  ...
  if m:
    ...
    if verbose:
      print >>sys.stderr, "Checking",f, "BAD (no checksum in XML)"
    else:
      print f
  h.close()
